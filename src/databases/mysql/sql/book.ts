...
const INSERT_BOOKS: (title: string, description: string) => Promise<number> = (
  title,
  description
) => mdb($.env.config.tc_book_name).insert({ title, description });

const SELECT_BOOKS: (id: number) => Promise<Book | undefined> = (id) =>
  mdb($.env.config.tc_book_name).where({ id }).first();

const SELECT_ALL_BOOKS = (
  limit: number = 10,
  page: number = 1
): Promise<Book[]> => {
  const query = mdb($.env.config.tc_book_name).select('*');
...
const DROP_BOOKS_TABLE: () => Promise<void> = () =>
  mdb.schema.dropTableIfExists($.env.config.tc_book_name);

const DELETE_BOOK: (id: number) => Promise<number> = (id) =>
  mdb($.env.config.tc_book_name).where({ id }).del();

...
export const updateTable = async () => {
  try {
    ...
    if (hasTable) {
      ...
      const getUniqueBackupTableName = async (baseName: string, count = 1): Promise<string> => {
        const tableName = count === 1 ? baseName : `${baseName}_${count}`;
        const exists = await mdb.schema.hasTable(tableName);
        return exists ? getUniqueBackupTableName(baseName, count + 1) : tableName;
      };

      backupTableName = await getUniqueBackupTableName(`${config.tc_book_name}_backup`);

      await mdb.raw(`RENAME TABLE ?? TO ??`, [config.tc_book_name, backupTableName]);

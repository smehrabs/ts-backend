...
import df_config from './defaults'
...
const schema = Joi.object({
    ...
    mysql_password: Joi.string().min(8).max(255).allow('') /* allow empty for Mysql password */.default(df_config.env.mysql_password),
    database_name: Joi.string().min(1).max(255).default(df_config.env.database_name),
    mysql_multipleStatements: Joi.boolean().default(df_config.env.mysql_multipleStatements),
    SECRET_KEY: Joi.string().min(10).required(),/* no default for SECRET_KEY */
    admin_user: Joi.string().min(1).max(255).required(), /* no default for admin user */
    admin_pass: Joi.string().min(6) /* admin password must be strong */.max(255).required(),
